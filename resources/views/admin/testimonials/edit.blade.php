        ...
        <div class="bg-white rounded-xl border border-gray-200/60 shadow-sm p-6 space-y-5">
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="4" required
                    class="w-full rounded-lg border-gray-200 focus:ring-[#DF5E1D] focus:border-[#DF5E1D] text-sm"
                    placeholder="What the customer said...">{{ old('content', $testimonial->content) }}</textarea>
                @error('content')
                    <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                @if ($testimonial->photo)
                    <div class="mb-3">
                        <img src="{{ Storage::url($testimonial->photo) }}" alt="Current photo" class="w-20 h-20 rounded-full object-cover border border-gray-200">
                    </div>
                @endif
                ...
                <p class="text-xs text-gray-500 mt-1">Max 2MB. JPG, PNG, or WebP. Leave empty to keep current photo.</p>
                @error('photo')
                    <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
