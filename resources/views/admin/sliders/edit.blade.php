...
@section('heading', 'Edit Hero Slider')
        ...
        <div class="bg-white rounded-xl border border-gray-200/60 shadow-sm p-6 space-y-5">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                    class="block w-full px-4 py-3 rounded-lg border-gray-200 bg-gray-50/50 focus:bg-white focus:border-[#DF5E1D] focus:ring-2 focus:ring-[#DF5E1D]/20 text-sm transition-colors"
                    placeholder="Slider description text...">{{ old('description', $slider->description) }}</textarea>
                @error('description')
                    <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                @if ($slider->image)
                    <div class="mb-3">
                        <img src="{{ $slider->image_url }}" alt="Current image" class="w-full max-h-40 rounded-lg object-cover border border-gray-200">
                    </div>
                @endif
                ...
                <p class="text-xs text-gray-500 mt-1">Max 5MB. JPG, PNG, or WebP. Leave empty to keep current image.</p>
                @error('image')
                    <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
