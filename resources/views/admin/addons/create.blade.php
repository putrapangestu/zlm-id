...
@section('title', 'Tambah Paket Add-On / Bundling')
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <h1 class="text-2xl font-bold text-gray-900 flex items-center gap-2">
                <iconify-icon icon="solar:add-circle-bold-duotone" class="text-[#DF5E1D] text-3xl"></iconify-icon>
                Tambah Paket Add-On / Bundling Baru
            </h1>
        </div>
    </div>
        ...
        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="Contoh: PAKET HEMAT, +ANTIGORES, Non Bundle" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#DF5E1D] focus:ring-1 focus:ring-[#DF5E1D] @error('name') border-rose-500 @enderror" required>
                    @error('name')
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="price" value="{{ old('price', 0) }}" min="0" step="1000" placeholder="0" class="w-full pl-12 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm font-mono font-bold focus:outline-none focus:border-[#DF5E1D] focus:ring-1 focus:ring-[#DF5E1D] @error('price') border-rose-500 @enderror" required>
                    </div>
            ...
            <div>
                ...
                <textarea name="description" rows="3" placeholder="Jelaskan isi dan bonus dari paket ini (contoh: Termasuk Mouse Wireless, Mousepad, dan Tas Laptop)..." class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#DF5E1D] focus:ring-1 focus:ring-[#DF5E1D]">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-xs text-rose-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" value="{{ old('sort_order', 0) }}" min="0" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#DF5E1D] focus:ring-1 focus:ring-[#DF5E1D]">
            ...
            <div class="pt-4 border-t border-gray-100 flex flex-col sm:flex-row gap-6">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_recommended" value="1" {{ old('is_recommended') ? 'checked' : '' }} class="w-4 h-4 text-[#DF5E1D] rounded border-gray-300 focus:ring-[#DF5E1D]">
                    <div>
                ...
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="w-4 h-4 text-[#DF5E1D] rounded border-gray-300 focus:ring-[#DF5E1D]">
                    <div>
