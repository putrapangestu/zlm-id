...
<div class="max-w-4xl mx-auto">
    ...
    <div class="flex items-center gap-2 text-sm text-gray-400 mb-6">
        ...
        <span class="text-[#363230] font-medium">Edit: {{ $user->name }}</span>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200/60 shadow-sm p-6 sm:p-8">
        <div class="mb-6 pb-4 border-b border-gray-100 flex items-center justify-between">
            <div>
                <h2 class="text-lg font-semibold text-[#363230]">Edit Informasi & Hak Akses</h2>
                <p class="text-xs text-gray-500 mt-1">Perbarui role, permission karyawan, dan data keanggotaan.</p>
            </div>
            <span class="px-3 py-1 text-xs font-semibold rounded-full @if($user->hasRole('admin')) bg-purple-50 text-purple-700 border border-purple-200 @elseif($user->hasRole('karyawan')) bg-blue-50 text-blue-700 border border-blue-200 @else bg-gray-100 text-gray-700 @endif">
        ...
        <form method="POST" action="{{ route('admin.users.update', $user) }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                {{-- Member Tier --}}
                <div>
                    <label for="member_tier" class="block text-sm font-medium text-[#363230] mb-1.5">Member Tier</label>
                    <select id="member_tier" name="member_tier"
                        class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-[#363230] focus:outline-none focus:border-[#DF5E1D]/30 focus:ring-4 focus:ring-[#DF5E1D]/10 transition-all">
                        <option value="bronze" @selected(old('member_tier', $user->member_tier) === 'bronze')>Bronze (0% Diskon)</option>
                        <option value="silver" @selected(old('member_tier', $user->member_tier) === 'silver')>Silver (1.5% Diskon)</option>
                        <option value="gold" @selected(old('member_tier', $user->member_tier) === 'gold')>Gold (3% Diskon)</option>
                        <option value="platinum" @selected(old('member_tier', $user->member_tier) === 'platinum')>Platinum (5% Diskon)</option>
                    </select>
                </div>

                {{-- Member Points --}}
                <div>
                    <label for="member_points" class="block text-sm font-medium text-[#363230] mb-1.5">Poin Member</label>
                    <input type="number" id="member_points" name="member_points" value="{{ old('member_points', $user->member_points) }}" min="0"
                        class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-[#363230] focus:outline-none focus:border-[#DF5E1D]/30 focus:ring-4 focus:ring-[#DF5E1D]/10 transition-all">
                </div>

                {{-- Password --}}
                <div>
                    <label for="password" class="block text-sm font-medium text-[#363230] mb-1.5">Password Baru (Opsional)</label>
                ...
                {{-- Confirm Password --}}
                <div>
                    <label for="password_confirmation" class="block text-sm font-medium text-[#363230] mb-1.5">Konfirmasi Password Baru</label>
