...
<div class="max-w-2xl mx-auto">
    ...
    <div class="flex items-center gap-2 text-sm text-gray-400 mb-6">
        ...
        <iconify-icon icon="solar:alt-arrow-right-linear" style="stroke-width: 1.5;"></iconify-icon>
        <span class="text-[#363230] font-medium">Edit</span>
    </div>

    <div class="bg-white rounded-2xl border border-gray-200/60 shadow-sm p-6 sm:p-8">
        <div class="flex items-center gap-3 mb-6">
            <div class="w-10 h-10 rounded-xl bg-[#DF5E1D]/10 flex items-center justify-center text-sm font-semibold text-[#DF5E1D]">
                {{ strtoupper(substr($user->name, 0, 2)) }}
            </div>
            <div>
                <h2 class="text-lg font-semibold text-[#363230]">Edit User</h2>
        ...
        <form method="POST" action="{{ route('admin.users.update', $user) }}" class="space-y-5">
            ...
            {{-- Password --}}
            <div>
                <label for="password" class="block text-sm font-medium text-[#363230] mb-1.5">Password <span class="text-gray-400 font-normal">(leave blank to keep current)</span></label>
